...
*   **定期間隔実行:** 設定ファイルで指定された**時間間隔**（デフォルト: 300秒、mini 180秒）ごとに、自動コミット処理を実行します。
...
*   **LLMによるコミットメッセージ生成:** コミット対象の差分 (`git diff --staged`) を基に、LLM (OpenAI API, ローカルLLMなど) を利用して適切なコミットメッセージを自動生成します。生成されたメッセージには `[Auto commit]` のような接頭辞を付与し、手動コミットとの区別を容易にします。
*   **カウントダウン表示:** `start` コマンドを実行したターミナルに、次回の自動コミット実行までの残り時間を秒単位でカウントダウン表示します。
*   **自動プッシュ (オプション):** 設定ファイル (`autoPush: true`) または `init` 時の対話で有効にした場合、自動コミット成功後に自動で `git push` を実行します。
...
*   **コア機能 (依存ライブラリは最小限に):**
    *   **設定ファイル処理:** プロジェクトルート下の `.auto-committer/config.yaml` ファイルを `js-yaml` ライブラリで読み込み、実行間隔 (`commitIntervalSeconds`)、LLM設定、自動プッシュ設定 (`autoPush`) などを取得します。環境変数は `.auto-committer/.env` から `dotenv` で読み込みます。
    *   **定期間隔実行ロジック:** Node.js の `setInterval` を1秒ごとに実行します。内部で残り時間を管理し、ターミナルにカウントダウンを表示（同一行上書き）。残り時間が0になったらコミット処理（ステージング → 差分取得 → LLM連携 → コミット → オプションでプッシュ）を実行し、タイマーをリセットします。
    *   **Git 操作:** Node.js 標準の `child_process` モジュール (`spawn` または `exec`) を利用して `git` コマンド (`add .`, `diff --staged`, `commit`, `push`) を直接実行します。コミット対象は `.gitignore` に従います。
    *   **初期化処理 (`init` コマンド):**
        *   `.auto-committer/` ディレクトリを作成します。
        *   `.auto-committer/config.yaml` (watchPaths を含まない) と `.auto-committer/.env.example` のテンプレート生成。
        *   `.gitignore` への `.auto-committer/.env` 追記。
        *   **対話形式での VS Code 自動起動設定:** ユーザーに VS Code でワークスペースを開いた際に自動で `start` コマンドを実行するか確認し、同意があれば `.vscode/tasks.json` を自動生成または追記。
        *   **対話形式での自動プッシュ設定:** ユーザーに自動プッシュを有効にするか確認し、同意があれば生成する `config.yaml` の `autoPush` を `true` に設定。
    *   **LLM 連携 (初回は OpenAI):**
        *   OpenAI API を利用する場合は `openai` ライブラリを使用。APIキーは `.auto-committer/.env` ファイルから `dotenv` で読み込みます。
        *   `git diff --staged` の結果をプロンプトに含め、コミットメッセージ生成を依頼します。
        *   将来的には Ollama 等のローカルLLM連携も検討。
    *   **自動プッシュ処理:** `autoPush` が有効な場合、コミット後に `git push` コマンドを実行します。現在のブランチ名を自動検出し、`origin` リモートにプッシュします (エラーハンドリング含む)。
*   **実行形態 (CLI ツール):**
    ...
    *   **代替利用方法:** VS Code を使用しない環境や、手動で起動したい場合は、プロジェクトディレクトリで `npx auto-committer start` コマンドを実行し、定期間隔コミットプロセスを**フォアグラウンドで**起動することも可能です。
...
2.  **初期セットアップ (プロジェクトごと):**
    ...
    *   このコマンドは以下の処理を行います:
        ...
        *   **設定ファイルテンプレートの生成 (in `.auto-committer/`):**
            *   `config.yaml`: 実行間隔、LLM設定、自動プッシュ設定などを記述するためのテンプレート (`watchPaths` は削除)。
...
7.  **実行停止:**
    ...
    *   **手動で起動した場合:** `start` コマンドを実行したターミナルで `Ctrl+C` を押してプロセスを停止します。(`npx auto-committer stop` コマンドは提供しません。)
